...
namespace Alsing.Windows.Forms.SyntaxBox
{
    ...
    public class EditViewControl : SplitViewChildControl
    {
                            ...
                            Replace(@"
      {
		...
		public delegate void AutolistTextInsertedDelegate(string InsertedText);
		public AutolistTextInsertedDelegate AutolistTextInserted = null;
		...
		public void InsertAutolistText()
		{
			...
			if (Caret.CurrentWord != null && Caret.CurrentWord.Type == WordType.Word)
			{
			...
			{
				if (AutoListStartPos == null) return;
			...
			if (AutolistTextInserted != null) AutolistTextInserted(AutoList.SelectedText);
			SetFocus();
		} 
        ...
        public bool AutoListVisible
        {
            set
            {
                ...
                if (value)
                {
					...
					p.X += (Painter as NativePainter).CaretX;
					p.Y += (Painter as NativePainter).CaretY + 12;
					
					AutoList.Location = p; //p.X, p.Y);
					AutoList.Show();
